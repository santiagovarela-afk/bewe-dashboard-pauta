...
import { fetchTopQueries, isGSCConfigured } from "@/lib/google-search-console";
...
export async function GET(req: NextRequest) {
  ...
  const limit = parseInt32(req.nextUrl.searchParams.get("limit"), 50, 1000);
  try {
    const data = await fetchTopQueries({ days, limit });

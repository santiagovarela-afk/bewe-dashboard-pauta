...
import type { AeoRun, AeoStats } from "@/lib/aeo";
...
/* ─────────────────────────────────────────────────────────────
 * AEO Section · cómo aparece Bewe en respuestas de LLMs
 * ────────────────────────────────────────────────────────────*/
...
function AeoSection() {
  ...
  const lastRun = data?.latest?.runAt
    ? new Date(data.latest.runAt).toLocaleString("es", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })
    : null;

  return (
    <section className="space-y-4">
      <SectionHeader
        title="AEO · cómo aparece Bewe en LLMs"
        sub="Answer Engine Optimization · análisis vía Gemini · gratis con tu API key existente"
        right={
          <>
            {lastRun && (
            ...
            <Button onClick={runAnalysis} size="sm" variant="glow" disabled={running}>
              {running ? <Loader2 className="size-3.5 animate-spin" /> : <Sparkles className="size-3.5" />}
              {running ? "Corriendo…" : data?.hasData ? "Nuevo análisis" : "Correr análisis"}
            </Button>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-[12px] text-muted-foreground leading-relaxed max-w-[640px]">
                AEO mide cómo aparece Bewe cuando alguien le pregunta a un LLM por software de gestión.
                Corremos ~30 prompts (belleza, comercio, servicios) contra Gemini, detectamos si Bewe
                aparece, en qué posición, qué competidores se mencionan y qué industrias adyacentes
                podríamos cubrir.
              </p>
      ...
      {data?.hasData && data.stats && (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <AeoKpi
              label="Visibility"
              value={`${data.stats.visibilityPct}%`}
              sub={`Bewe en ${Math.round((data.stats.visibilityPct / 100) * data.stats.totalPrompts)} / ${data.stats.totalPrompts} prompts`}
              accent="var(--brand-violet)"
            />
            <AeoKpi
              label="Pos. media"
              value={data.stats.avgPosition !== null ? data.stats.avgPosition.toFixed(1) : "—"}
              sub={data.stats.avgPosition !== null ? "Posición en listas" : "Sin listas detectadas"}
              accent="var(--brand-cyan)"
            />
            <AeoKpi
              label="Competidores"
              value={String(data.stats.topCompetitors.length)}
              sub={data.stats.topCompetitors[0] ? `top: ${data.stats.topCompetitors[0].name}` : "—"}
              accent="var(--brand-ember)"
            />
            <AeoKpi
              label="Industrias adyacentes"
              value={String(data.stats.topIndustries.length)}
              sub={data.stats.topIndustries[0] ? `top: ${data.stats.topIndustries[0].name}` : "—"}
              accent="var(--brand-lime)"
            />
          </div>

          {/* Top competitors + industries */}
          <div className="grid lg:grid-cols-2 gap-3">
            <TextureCard className="p-5">
              <div className="flex items-center gap-2 mb-3">
                <Building2 className="size-4 text-[hsl(var(--brand-ember))]" />
                <h3 className="text-[12px] font-bold uppercase tracking-[0.12em] text-muted-foreground">
                  Top competidores nombrados
                </h3>
              </div>
              {data.stats.topCompetitors.length === 0 ? (
                <p className="text-[11px] text-muted-foreground">No se detectaron competidores en este run.</p>
              ) : (
                <div className="space-y-1.5">
                  {data.stats.topCompetitors.map((c) => {
                    const max = data.stats!.topCompetitors[0].count;
                      ...
                      <div key={c.name} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 h-1.5 bg-secondary/50 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${pct}%`,
                              background: "hsl(var(--brand-ember))",
                            }}
                          />
                        </div>
            ...
            <TextureCard className="p-5">
              ...
              <p className="text-[10px] text-muted-foreground mt-3 leading-snug">
                Oportunidades de expansión · industrias que los LLMs ya asocian a software de gestión.
              </p>
            </TextureCard>
          </div>
          ...
          <TextureCard className="p-0 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-background/40">
              ...
              <Badge variant="outline" className="font-mono">
                {data.latest?.results.length ?? 0} prompts
              </Badge>
            </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            >
                              {r.mentionsBewe ? `Bewe ✓${r.bewePosition ? ` #${r.bewePosition}` : ""}` : "Bewe ✗"}
                            </Badge>
                            {r.errored && (
          ...
          {data.history.length > 1 && (
            <TextureCard className="p-4">
              <div className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground mb-2">
                Histórico · últimos {data.history.length} runs
              </div>
              <div className="flex flex-wrap gap-2">
                {data.history.map((h) => (
                  ...
                  >
                    {new Date(h.runAt).toLocaleString("es", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                    {" · "}
...
function AeoKpi({ label, value, sub, accent }: { label: string; value: string; sub: string; accent: string }) {
  return (
    <TextureCard className="p-4">
      <div className="text-[9px] uppercase tracking-[0.12em] text-muted-foreground">{label}</div>
      <div
        className="font-mono font-bold text-2xl tabular leading-none mt-1"
        style={{ color: `hsl(${accent})` }}
      >
        {value}
      </div>
      <div className="text-[10px] text-muted-foreground mt-1.5 truncate">{sub}</div>
    </TextureCard>
  );
}
